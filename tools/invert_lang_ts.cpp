...
int main(int argc, char *argv[])
{
	...
	const QStringList langs = QStringList() << "ja" << "zh";

	QFile f("translations/hyne_en_fr.ts");
	...
	for (const QString &lang: langs) {
		QFile f2(QString("translations/hyne_%1_fr.ts").arg(lang));
		f2.open(QIODevice::ReadOnly);
		QXmlStreamReader reader2(&f2);

		QFile f3(QString("translations/hyne_%1_en.ts").arg(lang));
	...
	QFile f("translations/hyne_en.ts");
	f.open(QIODevice::ReadOnly);
	QXmlStreamReader reader(&f);
